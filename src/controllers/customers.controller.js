...
const handleError = (error, res) => {
  ...
  if (error.name === 'SequelizeDatabaseError') {
    ...
    errorMessage = `Database error: ${error.original ? error.original.sqlMessage : error.message}`;
...
exports.getAllCustomers = async (req, res) => {
  try {
    ...
    if (customerSince) {
      try {
        ...
        if (!isNaN(sinceDate.getTime())) {
          ...
          console.log(`Applying customer since filter: created on or after ${customerSince}`);
        }
      } catch (e) {
    ...
    if (sort) {
      ...
      console.log(`Parsed sort: field = "${field}", direction = "${direction}"`);
      ...
      switch (field) {
        ...
        default:
          console.log(`Unknown sort field: "${field}", defaulting to created_at`);
          orderField = 'created_at'; // Default sort
      }
      
      console.log(`Sorting by field: "${field}" mapped to database column: "${orderField}"`);
      queryOptions.order = [[orderField, direction.toLowerCase() === 'desc' ? 'DESC' : 'ASC']];
    } else {
      queryOptions.order = [['name', 'ASC']];
    }
    
    // Get customers with count
    const { count, rows: customers } = await Customer.findAndCountAll(queryOptions);
    ...
    const invoices = await Invoice.findAll({
      ...
      include: [
        { 
          ...
          attributes: ['id', 'service_id', 'service_name', 'price', 'quantity', 'total'] 
        }
      ]
    });
