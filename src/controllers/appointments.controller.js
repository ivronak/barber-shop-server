...
exports.getAllAppointments = async (req, res) => {
  try {
    const { date, staffId, customerId, status, sort, page = 1, limit = 10 } = req.query;
    ...
    const { count, rows: appointments } = await Appointment.findAndCountAll(queryOptions);
...
exports.createAppointment = async (req, res) => {
  ...
  try {
    ...
    if (!customer_id || !staff_id || !date || !time || !services || !services.length) {
    ...
    const slotAvailable = await checkAvailability(staff_id, date, time, endTimeString);
    if (!slotAvailable) {
      await transaction.rollback();
      return res.status(409).json({
        success: false,
        message: 'The selected time slot is no longer available. Please choose a different slot.'
      });
    }
...
exports.updateAppointment = async (req, res) => {
  ...
  try {
    ...
    const { id } = req.params;
    const { status, notes, services: newServiceIds, products: newProductIds, tipAmount, paymentMethod } = req.body;
    
    console.log(`[INVOICE-DEBUG] Starting appointment update: ID=${id}, Status=${status}`);
    ...
    console.log(`[INVOICE-DEBUG] Found appointment: ${appointment.id}, Current Status=${originalStatus}, New Status=${status}`);
    ...
    if (Array.isArray(newServiceIds)) {
      ...
      for (const serviceId of newServiceIds) {
        ...
        if (!svc) {
          await transaction.rollback();
          return res.status(400).json({ success: false, message: `Service ${serviceId} not found` });
        }
      ...
      const newTotalAmount = updatedServiceDetails.reduce((t, s) => t + parseFloat(s.price), 0);
      await appointment.update({ total_amount: newTotalAmount }, { transaction });
    }
    ...
    console.log(`[INVOICE-DEBUG] Appointment status updated: ${status || appointment.status}`);
    ...
    if (status === 'completed' && originalStatus !== 'completed') {
      console.log(`[INVOICE-DEBUG] Status is changing from '${originalStatus}' to 'completed' - starting invoice generation process`);
      ...
      try {
        ...
        console.log(`[INVOICE-DEBUG] Updated customer visit stats for customer ID: ${appointment.customer_id}`);
        ...
        if (existingInvoice) {
          console.log(`[INVOICE-DEBUG] Invoice already exists for this appointment: ${existingInvoice.id}`);
        }
        
        if (!existingInvoice) {
          console.log(`[INVOICE-DEBUG] No existing invoice found - creating new invoice`);
          ...
          console.log(`[INVOICE-DEBUG] Got business settings - tax rate: ${taxRate}%`);
          ...
          console.log(`[INVOICE-DEBUG] Active GST Rate found: ${activeGSTRate ? 'Yes' : 'No'}`);
          
          // Calculate invoice details
          const invoiceServices = [];
          let subtotal = 0;
          
          const servicesForInvoice = updatedServiceDetails || appointment.appointmentServices;
          console.log(`[INVOICE-DEBUG] Processing ${servicesForInvoice.length} services for the invoice`);
          ...
          for (const svc of servicesForInvoice) {
            ...
            const serviceId = (svc && typeof svc === 'object' && 'service_id' in svc)
              ? svc.service_id
              : svc.id;
          ...
          if (Array.isArray(newProductIds) && newProductIds.length > 0) {
            console.log(`[INVOICE-DEBUG] Processing ${newProductIds.length} products for the invoice`);
          ...
          const finalTaxRate = activeGSTRate ? parseFloat(activeGSTRate.total_rate) : parseFloat(taxRate);
          ...
          console.log(`[INVOICE-DEBUG] Invoice calculation: Subtotal=${subtotal.toFixed(2)}, Tax Rate=${finalTaxRate.toFixed(2)}%, Tax Amount=${taxAmount.toFixed(2)}, Total=${total.toFixed(2)}`);
          ...
          if (activeGSTRate && activeGSTRate.components) {
            ...
            console.log(`[INVOICE-DEBUG] Created ${taxComponents.length} tax components`);
          ...
          console.log(`[INVOICE-DEBUG] Invoice created successfully with ID: ${invoice.id}`);
          
          // Create all invoice services in a single bulk operation
          if (invoiceServices.length > 0) {
            console.log(`[INVOICE-DEBUG] Creating ${invoiceServices.length} invoice service records`);
            ...
            const uniqueStaff = [...new Set(invoiceServices.map(s=>s.staff_id).filter(Boolean))];
            if (tipAmtNum > 0 && uniqueStaff.length > 0) {
              ...
              for (let i = 0; i < uniqueStaff.length; i++) {
                ...
                const staffShare = isLast ? +(tipAmtNum - allocatedStaff).toFixed(2) : equalShare;
                allocatedStaff = +(allocatedStaff + staffShare).toFixed(2);
                const staffLines = await InvoiceService.findAll({ where: { invoice_id: invoiceId, staff_id: uniqueStaff[i] }, transaction });
                const linesTotal = staffLines.reduce((s,l)=> s + Number(l.total||0), 0);
                if (linesTotal <= 0) {
                  const perLine = +(staffShare / Math.max(1, staffLines.length)).toFixed(2);
                  let allocLine = 0;
                  for (let j = 0; j < staffLines.length; j++) {
                    const lastLine = j === staffLines.length - 1;
                    const amount = lastLine ? +(staffShare - allocLine).toFixed(2) : perLine;
                    await staffLines[j].update({ tip_amount: amount }, { transaction });
                  ...
                  for (let j = 0; j < staffLines.length; j++) {
                    const lastLine = j === staffLines.length - 1;
                    const portion = lastLine ? +(staffShare - allocLine).toFixed(2) : +((staffShare * (Number(staffLines[j].total||0) / linesTotal)).toFixed(2));
                    await staffLines[j].update({ tip_amount: portion }, { transaction });
                    allocLine = +(allocLine + portion).toFixed(2);
                  }
                }
              }
              // Update invoice total with allocated tip
              const [sumRow] = await InvoiceService.findAll({ where: { invoice_id: invoiceId }, attributes: [[Invoice.sequelize.fn('sum', Invoice.sequelize.col('tip_amount')), 'tipSum']], raw: true, transaction });
              const tipSum = parseFloat(sumRow?.tipSum || 0);
              await invoice.update({ total: parseFloat((subtotal + taxAmount + tipSum).toFixed(2)) }, { transaction });
            }
            
            // Bulk create all invoice services
            await InvoiceService.bulkCreate(serviceRecords, { transaction });
            console.log(`[INVOICE-DEBUG] Invoice services created successfully in bulk`);
          }
          
          // Create all tax components in a single bulk operation
          if (taxComponents.length > 0) {
            console.log(`[INVOICE-DEBUG] Creating ${taxComponents.length} tax components`);
            ...
            console.log(`[INVOICE-DEBUG] Tax components created successfully in bulk`);
          }
          
          // Create all invoice products in a single bulk operation
          if (productRecords.length > 0) {
            await sequelize.models.InvoiceProduct.bulkCreate(productRecords.map(p => ({
              ...p,
              invoice_id: invoiceId,
            })), { transaction });
            console.log(`[INVOICE-DEBUG] Invoice products created successfully in bulk`);
          }
          
          // Update customer total_spent in the same transaction
          console.log(`[INVOICE-DEBUG] Updating customer total_spent with formatted total: ${parseFloat(total).toFixed(2)}`);
          await Customer.update({
            total_spent: sequelize.literal(`total_spent + ${parseFloat(total).toFixed(2)}`),
            last_visit: appointment.date
          }, {
            where: { id: appointment.customer_id },
            transaction
          });
          console.log(`[INVOICE-DEBUG] Customer total_spent updated successfully in transaction`);
        }
      } catch (invoiceError) {
        console.error(`[INVOICE-DEBUG] Error in invoice generation process:`, invoiceError);
        throw invoiceError; // Re-throw to trigger the transaction rollback
      }
    } else {
      console.log(`[INVOICE-DEBUG] Status condition not met for invoice generation: status=${status}, originalStatus=${originalStatus}`);
    }
    ...
    if (req.user) {
      await ActivityLog.create({
        ...
        details: `Updated appointment #${id} status to ${status || appointment.status}`
      }, { transaction });
    ...
    console.log(`[INVOICE-DEBUG] Reloading appointment with invoice information`);
    const updatedAppointment = await Appointment.findByPk(id, {
      include: [
        { association: 'customer' },
        { association: 'staff', include: ['user'] },
        { association: 'appointmentServices', include: ['service'] },
        { association: 'invoice', attributes: { exclude: ['staff_id', 'staff_name'] } } // Also include the invoice if it was created
      ]
    });
    
    console.log(`[INVOICE-DEBUG] Has invoice after reload: ${updatedAppointment.invoice ? 'Yes' : 'No'}`);
    ...
    try {
      ...
      console.error('[INVOICE-DEBUG] Transaction rollback error:', rollbackError);
    }
...
exports.getAdminAppointments = async (req, res) => {
  try {
    ...
    // Add service filter
    const serviceIds = Array.isArray(serviceId) ? serviceId : serviceId ? [serviceId] : null;
    ...
    if (sort) {
      const [field, direction] = sort.split('_');
      appointmentsQuery.order = [[field, direction === 'desc' ? 'DESC' : 'ASC']];
    } else {
      appointmentsQuery.order = [['date', 'ASC'], ['time', 'ASC']];
    }
...
exports.getStaffAppointments = async (req, res) => {
  try {
    ...
    console.log('Fetching appointments with query:', JSON.stringify(appointmentsQuery.where));
