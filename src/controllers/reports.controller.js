...
const getDashboardStats = async (req, res) => {
  try {
    ...
    const tipsDiscountsSummary =
      tipsSummaryRows && tipsSummaryRows[0]
        ? tipsSummaryRows[0]
        : {
            totalTips: 0,
            totalDiscounts: 0,
    ...
    addRows(svcStaffRows);
    addRows(prodStaffRows);
  ...
  try {
    ...
    const revenueData = await Invoice.findAll({
      attributes: [
        ...
        [
          literal(
            "SUM((SELECT COALESCE(SUM(isvc.tip_amount),0) FROM invoice_services isvc WHERE isvc.invoice_id = Invoice.id))"
          ),
          "tips",
        ],
  ...
  try {
    ...
    const servicesData = await InvoiceService.findAll({
      attributes: [
        ...
        [
          literal(
            "SUM(CASE WHEN invoice.subtotal > 0 THEN invoice.discount_amount * (InvoiceService.total / invoice.subtotal) ELSE 0 END)"
          ),
          "discounts",
        ],
    ...
    const staffWithCommission = await Promise.all(
      staffRevenueRows.map(async (row) => {
        ...
        const productCommission =
          (await InvoiceProduct.sum("commission_amount", {
            where: { staff_id },
            include: [
              {
                model: Invoice,
        ...
        const totalCommission =
          parseFloat(serviceCommission) + parseFloat(productCommission);
...
const getTipsAndDiscountsReport = async (req, res) => {
  try {
    ...
    if (staffId) {
      const idsFromSvc = await InvoiceService.findAll({
        attributes: ["invoice_id"],
        where: { staff_id: staffId },
      });
      const idsFromProd = await InvoiceProduct.findAll({
        attributes: ["invoice_id"],
        where: { staff_id: staffId },
      });
      invoiceIdsForStaff = [
        ...new Set([
          ...idsFromSvc.map((r) => r.invoice_id),
          ...idsFromProd.map((r) => r.invoice_id),
        ]),
      ];
      if (invoiceIdsForStaff.length === 0) {
        invoiceIdsForStaff = ["no-match"];
      }
    }
    ...
    const staffBreakdown = await InvoiceService.findAll({
      attributes: [
        ...
        [
          literal(
            "SUM(invoice.discount_amount * (InvoiceService.total / NULLIF(invoice.subtotal,0)))"
          ),
          "totalDiscounts",
        ],
        ...
        [
          literal(
            "CASE WHEN SUM(invoice.total) > 0 THEN (SUM(InvoiceService.tip_amount) / SUM(invoice.total)) * 100 ELSE 0 END"
          ),
          "tipPercentage",
        ],
        [
          literal(
            "CASE WHEN SUM(invoice.subtotal) > 0 THEN (SUM(invoice.discount_amount * (InvoiceService.total / NULLIF(invoice.subtotal,0))) / SUM(invoice.subtotal)) * 100 ELSE 0 END"
          ),
          "discountPercentage",
        ],
      ],
...
const getRevenueByDayOfWeek = async (req, res) => {
  try {
    ...
    const revenueWithChange = revenueByDay.map((day) => {
      ...
      const previousDay = previousRevenueByDay.find(
        (prev) =>
          prev.getDataValue("numeric_day_of_week") ===
          dayData.numeric_day_of_week
      );

      const previousRevenue = previousDay
        ? previousDay.getDataValue("revenue")
        : 0;
...
const getAdvancedRevenueMetrics = async (req, res) => {
  try {
    ...
    const queryDateFrom = dateFrom
      ? new Date(dateFrom)
      : new Date(today.getFullYear(), today.getMonth(), 1); // Default: start of current month
    ...
    const periodLength = Math.floor(
      (queryDateTo - queryDateFrom) / (1000 * 60 * 60 * 24)
    );
    ...
    const revenuePerVisit =
      totalVisits > 0 ? currentPeriodRevenue / totalVisits : 0;
...
const getAdvancedStaffMetrics = async (req, res) => {
  try {
    ...
    const advancedStaffData = await Promise.all(
      staffMembers.map(async (staff) => {
        ...
        const serviceRevenueRow = await InvoiceService.findOne({
          attributes: [
            [fn("sum", col("InvoiceService.total")), "totalRevenue"],
          ],
        ...
        const productRevenueRow = await InvoiceProduct.findOne({
          attributes: [
            [fn("sum", col("InvoiceProduct.total")), "totalRevenue"],
          ],
          where: { staff_id: staff.id },
          include: [
            {
        ...
        const revenueFromServices = parseFloat(
          serviceRevenueRow?.totalRevenue || 0
        );
        const revenueFromProducts = parseFloat(
          productRevenueRow?.totalRevenue || 0
        );
        ...
        const utilization =
          scheduledHours > 0
            ? Math.min(
                100,
                Math.round((appointmentHours / scheduledHours) * 100)
              )
            : 0;
        ...
        const uniqueCustomers = new Set(
          customerAppointments.map((appt) => appt.customer_id)
        ).size;

        // Count customers with multiple appointments (rebooked)
        const customerAppointmentCount = {};
        customerAppointments.forEach((appt) => {
          customerAppointmentCount[appt.customer_id] =
            (customerAppointmentCount[appt.customer_id] || 0) + 1;
        });

        const rebookedCustomers = Object.values(
          customerAppointmentCount
        ).filter((count) => count > 1).length;
        const rebookRate =
          uniqueCustomers > 0
            ? Math.round((rebookedCustomers / uniqueCustomers) * 100)
            : 0;
        ...
        return {
          ...
          averageServiceTime:
            appointmentServices.length > 0
              ? Math.round(
                  appointmentServices.reduce((sum, s) => sum + s.duration, 0) /
...
const getAdvancedServiceMetrics = async (req, res) => {
  try {
    ...
    const advancedServiceData = await Promise.all(
      services.map(async (service) => {
        ...
        const revenue =
          (await InvoiceService.sum("InvoiceService.total", {
            include: [
              {
                model: Invoice,
                as: "invoice",
        ...
        try {
          const overheadSetting =
            await sequelize.models.BusinessSetting.findOne({
        ...
        const profitMargin =
          revenue > 0 ? ((revenue - estimatedCost) / revenue) * 100 : 0;
        ...
        const preferredByStaff = staffServices.map((staff) =>
        ...
        previousPeriodStart.setDate(
          previousPeriodStart.getDate() - periodLengthDays
        );
        ...
        const daysInPeriod =
          Math.round((queryDateTo - queryDateFrom) / (1000 * 60 * 60 * 24)) ||
          1;
        ...
        const maxPossibleBookings = Math.floor(
          totalCapacityMinutes / serviceDuration
        );
        ...
        const fallbackCapacity =
          daysInPeriod * availableStaff.length * reasonableCapacityPerDay;

        // Use calculated capacity if available, otherwise fallback
        const effectiveCapacity =
          maxPossibleBookings > 0 ? maxPossibleBookings : fallbackCapacity;
        ...
        const servicesPerAppointment = await AppointmentService.findAll({
          attributes: [
            "appointment_id",
            [fn("COUNT", col("appointment_id")), "service_count"],
          ],
          include: [
            {
              model: Appointment,
...
const getStaffPerformanceMetrics = async (req, res) => {
  try {
    ...
    if (
      (!targetStaffId || targetStaffId === req.user?.id) &&
      req.user?.staffId
    ) {
    ...
    const commissionPercentage = staffRecord
